...
composition_data <- seine_tags %>%
  ...
  mutate(Common_Name = fct_lump_n(Common_Name, n = 7, w = Frequency, other_level = "Otro")) %>%
  mutate(Common_Name = fct_relevel(Common_Name, "Otro")) %>%
  group_by(Month_Year, Common_Name) %>%
  ...
  mutate(
    ...
    Common_Name = case_when(
      Common_Name == "White mullet" ~ "Lisa blanca",
      Common_Name == "Tarpon" ~ "Sábalo",
      Common_Name == "Striped mullet" ~ "Lisa cabezuda",
      Common_Name == "Striped mojarra" ~ "Mojarra rayada",
      Common_Name == "Sheepshead" ~ "Sargo chopa",
      Common_Name == "Ladyfish" ~ "Machete",
      Common_Name == "Common snook" ~ "Róbalo",
      TRUE ~ Common_Name
    )
  )
# interactive plot
composition_plot <- plot_ly(
  ...
  text = ~ paste(
    "<b>Especies:</b>", Common_Name,
    "<br><b>Fecha:</b>", paste0(format(Month_Year, format = "%b %Y")),
    "<br><b>Frecuencia:</b>", Frequency
  ),
  hoverinfo = "text",
  textposition = "none"
) %>%
  layout(
    barmode = "stack",
    xaxis = list(title = "", gridcolor = "#cccccc"),
    yaxis = list(title = "Frecuencia", gridcolor = "#cccccc"),
    legend = list(title = list(text = "Nombre común")),
    hovermode = "closest",
    paper_bgcolor = "rgba(0,0,0,0)",
    plot_bgcolor = "rgba(0,0,0,0)"
  )
...
save(seine_data, rich_data_all, pit_sample_day, wbpal, dark2_pal, antenna_loc, 
     ...
     file = "data/rm-preprocessed-es.RData")
